...
const SACRED_MUSIC = [
  ...
  { name: "Ave Maria de Gounod", url: "https://cdn.pixabay.com/download/audio/2022/10/14/audio_985871b65e.mp3?filename=ave-maria-121347.mp3", desc: "Piano sereno e celestial" },
  { name: "Adoro Te Devote", url: "https://cdn.pixabay.com/download/audio/2022/11/02/audio_c352932906.mp3?filename=gregorian-chant-124097.mp3", desc: "Órgão gregoriano profundo" },
  { name: "Pange Lingua", url: "https://cdn.pixabay.com/download/audio/2022/11/22/audio_febc508520.mp3?filename=angelic-pad-126435.mp3", desc: "Harpa contemplativa" }
];
  ...
  return (
    <div className="p-6 pb-32 animate-fade-in min-h-screen relative">
      <div className="mb-8 bg-black/40 backdrop-blur-xl border border-sacred-gold/30 rounded-2xl p-5 shadow-2xl animate-fade-in">
        <div className="flex items-center gap-3 text-sacred-gold mb-4"><Music size={22} className="animate-pulse" /><h3 className="font-serif text-lg">Música Sacra para Meditação</h3><Volume2 size={18} className="text-sacred-gold/70" /></div>
        <div className="grid grid-cols-1 gap-3">
          {SACRED_MUSIC.map((track, i) => (
            <button key={i} onClick={() => toggleMusic(i)} className={`p-4 rounded-xl border flex items-center justify-between transition-all ${selectedMusic === i && isPlaying ? 'bg-sacred-gold/20 border-sacred-gold text-sacred-gold shadow-lg shadow-sacred-gold/30' : 'bg-white/5 border-white/10 hover:border-sacred-gold/40 text-gray-300'}`}>
              <div className="text-left"><div className="font-medium font-serif flex items-center gap-2">{track.name}{track.special && <Sparkles size={12} className="text-red-500 animate-pulse" />}</div><div className="text-xs opacity-70">{track.desc}</div></div>
      ...
      {loadingPlans ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center space-y-6 opacity-70"><Calendar size={48} className="text-gray-600" /><p className="font-serif text-gray-400 text-lg">"Crie seu primeiro plano com seu diretor espiritual"</p></div>
      ) : (
        <div className="space-y-4">
          {plans.map((plan) => {
      ...
      <button onClick={() => navigate('/planner/create')} className="fixed bottom-24 right-6 w-14 h-14 bg-gradient-to-r from-sacred-gold to-yellow-400 rounded-full flex items-center justify-center text-sacred-sapphire shadow-[0_0_20px_rgba(212,175,55,0.5)] hover:scale-110 active:scale-95 transition-all z-50"><Plus size={28} strokeWidth={3} /></button>
