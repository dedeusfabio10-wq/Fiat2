...
import {
  ...
  Search, X, RefreshCw, Loader2, Users, ArrowRight
} from 'lucide-react';
...
const ProfilePage: React.FC = () => {
  ...
  // Polling automático para confirmar pagamento
  useEffect(() => {
  ...
  const handleLogout = async () => {
    ...
    updateProfile({
      id: '', // CORRIGIDO — era isso que estava quebrando o build
      name: '',
      email: '',
      streak: 0,
      rosaries_prayed: 0,
  ...
  return (
    <div className={`min-h-screen pb-32 ${profile.nightModeSpiritual ? 'bg-black' : 'bg-slate-950'} text-white`}>
      {/* Header */}
      <div className="relative p-6 pb-8 bg-gradient-to-b from-black/40 to-transparent">
        ...
        <div className="flex flex-col items-center gap-4 mt-4">
          ...
          <div className="text-center space-y-1">
            {editName ? (
              <div className="flex gap-2 items-center justify-center">
                <Input
                  ...
                  className="h-8 w-40 text-center bg-black/40 border-fiat-gold/50"
                  autoFocus
              ...
              <h2
                className="text-2xl font-serif flex items-center justify-center gap-2 cursor-pointer"
                onClick={() => {
            ...
            {devotionalSaint && (
              <p className="text-fiat-gold text-sm flex items-center gap-1 font-serif italic">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">Crie ou entre em grupos de terço, novena e oração diária</p>
              </div>
            </div>
      ...
      <div className="p-6 space-y-6">
        {profile.is_premium ? (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl border border-white/10 p-5 relative overflow-hidden shadow-lg">
              <div className="absolute top-0 right-0 p-3 opacity-5"><Crown size={80} /></div>
              ...
              <div className="mt-4 pt-4 border-t border-white/5">
                <Button size="sm" variant="outline" className="w-full" onClick={() => setShowPremiumModal(true)}>
            ...
            {/* Santo de Devoção */}
            <Card className="bg-white/5 border-fiat-gold/20 cursor-pointer hover:border-fiat-gold/50 transition-all" onClick={() => setShowSaintModal(true)}>
              <div className="p-4 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-full bg-fiat-gold/10 flex items-center justify-center border border-fiat-gold/20">
                    {devotionalSaint ? <SaintIcon id={devotionalSaint.id} className="w-6 h-6" /> : <Heart size={20} />}
                  </div>
                  <div>
                    <h4 className="font-serif">Santo de Devoção</h4>
                    <p className="text-xs text-gray-400">{devotionalSaint ? devotionalSaint.name : 'Escolha seu intercessor'}</p>
                  </div>
                </div>
                <span className="text-fiat-gold text-xs font-bold uppercase px-3 py-1 rounded border border-fiat-gold/30">Alterar</span>
              </div>
            </Card>
            ...
            <div className="bg-white/5 p-4 rounded-xl border border-white/10">
              ...
              <div className="relative">
                ...
                />
                <PenLine size={14} className="absolute right-3 top-3 text-gray-600" />
              </div>
            </div>
            ...
            <div className="flex items-center justify-between bg-white/5 p-4 rounded-xl border border-white/10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-medium">Modo Noturno Espiritual</h4>
              ...
              >
                <div className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${profile.nightModeSpiritual ? 'translate-x-6' : ''}`} />
              </button>
            </div>
          </div>
        ) : (
          /* ÁREA NÃO PREMIUM */
          <div className="relative overflow-hidden rounded-xl bg-gradient-to-b from-gray-800 to-gray-900 border border-yellow-600/30 shadow-lg">
            <div className="bg-gradient-to-r from-yellow-700 via-yellow-600 to-yellow-500 p-4 flex items-center justify-between">
              <div className="flex items-center gap-2 text-white font-bold">
                <Crown size={20} fill="currentColor" /> <span className="font-serif">Premium</span>
              </div>
              <Lock size={16} className="text-yellow-100 opacity-70" />
            </div>
            <div className="p-6 space-y-4">
              <p className="text-gray-300 text-sm font-serif italic text-center leading-relaxed">
              ...
              <Button onClick={() => setShowPremiumModal(true)} variant="sacred" className="w-full shadow-[0_0_15px_rgba(212,175,55,0.2)]">
                Tornar-se Premium
              </Button>
              <div className="flex justify-center pt-2">
                <button onClick={handleManualCheck} disabled={isChecking} className="flex items-center gap-2 text-xs text-yellow-500/70 hover:text-yellow-400 uppercase tracking-wider font-medium">
        ...
        <div
          ...
          className="bg-white/5 border border-white/10 p-4 rounded-xl flex items-center justify-between cursor-pointer hover:bg-white/10 hover:border-fiat-gold/30 transition-all group"
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-serif font-medium">Sobre o Fiat</h3>
              <p className="text-xs text-gray-400">O significado do seu Sim</p>
            </div>
          </div>
          <ArrowRight className="w-5 h-5 text-gray-500 group-hover:text-fiat-gold transition-colors" />
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="bg-white/5 rounded-xl border border-white/5 overflow-hidden">
            <MenuRow icon={<BarChart3 size={18} />} label="Estatísticas" value={`${profile.rosaries_prayed} Terços`} />
            <MenuRow icon={<Settings size={18} />} label="Preferências do App" />
      ...
      {showAboutFiat && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6" onClick={() => setShowAboutFiat(false)}>
          <div className="absolute inset-0 bg-black/90 backdrop-blur-sm" />
          <div className="relative bg-[#0f172a] w-full max-w-md rounded-2xl border border-fiat-gold/40 shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="relative z-10 p-8 text-center space-y-7">
              ...
              <p className="text-gray-200 text-sm leading-relaxed px-4">
                Fiat é o seu <span className="text-fiat-gold">SIM diário</span> a Jesus.
              </p>
      ...
      {showSaintModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-[#0f172a] w-full max-w-md h-[80vh] rounded-2xl border border-fiat-gold/20 flex flex-col overflow-hidden">
...
const MenuRow: React.FC<{ icon: React.ReactNode; label: string; onClick?: () => void; value?: string; danger?: boolean }> = ({
  ...
  <div
    onClick={onClick}
    className={`flex items-center justify-between p-4 cursor-pointer hover:bg-white/5 transition-colors border-b border-white/5 last:border-0 ${danger ? 'text-red-400 hover:bg-red-900/10' : ''}`}
  >
    <div className="flex items-center gap-3">
      <div className="opacity-70">{icon}</div>
      <span className="font-medium text-sm">{label}</span>
    </div>
    {value ? <span className="text-xs text-gray-500">{value}</span> : !danger && <ArrowRight className="w-5 h-5 text-gray-600" />}
  </div>
);

const ThemeOption: React.FC<{ color: string; label: string; active: boolean; onClick: () => void; light?: boolean }> = ({
  color, active, onClick, light
}) => (
  <button
    onClick={onClick}
    className={`flex-1 h-10 rounded-lg transition-all relative ${color} ${active ? 'ring-2 ring-white scale-105' : 'opacity-60 hover:opacity-100'}`}
  >
