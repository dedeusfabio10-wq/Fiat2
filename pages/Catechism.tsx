...
import { 
  Crown, Lock, ChevronDown, ChevronUp, BookOpen, Sparkles, 
  Star, Cloud, Heart, Bell, Music, Users, Grape, Sun, Droplet,
  Loader2, RefreshCw 
} from 'lucide-react';
...
const CatechismPage: React.FC = () => {
  ...
  // Busca seções principais do catecismo (credo, sacramentos, mandamentos)
  const { data: sections, loading: loadingSections } = useContent();
  const catechismSections = sections
    .filter(s => ['credo', 'sacramentos', 'mandamentos'].includes(s.id))
    .sort((a, b) => a.order - b.order);

  // Busca dogmas marianos
  const { data: marianDogmas } = useContent('marian-dogmas');

  // Busca items detalhados de cada seção (quando expandir)
  const { data: allItems } = useContent();
  ...
  // Polling automático para quem está aguardando pagamento
  useEffect(() => {
  ...
  const getIcon = (iconName: string) => {
    const icons: Record<string, React.ReactNode> = {
      ...
      Bell: <Bell size={20} className="text-fiat-gold" />,
      Music: <Music size={20} className="text-fiat-gold" />,
      BookOpen: <BookOpen size={20} className="text-fiat-gold" />,
      Users: <Users size={20} className="text-fiat-gold" />,
      Grape: <Grape size={20} className="text-fiat-gold" />,
      Sun: <Sun size={20} className="text-fiat-gold" />,
      Droplet: <Droplet size={20} className="text-fiat-gold" />,
    };
    return icons[iconName] || <Sparkles size={20} className="text-fiat-gold" />;
  };

  if (isLoadingProfile || loadingSections) {
    return (
    ...
    <div className="p-6 pb-32 min-h-screen animate-fade-in">
      ...
      <div className="space-y-4">
        {catechismSections.map(section => {
          const items = allItems.filter(i => i.category === section.id);
          return (
                    ...
                    {items.length > 0 ? items.map(item => (
                      <div key={item.id} className="bg-black/20 p-4 rounded-lg border border-white/5">
                        ...
                        <p className="text-gray-300 text-sm leading-relaxed font-serif italic opacity-90">
                          {item.description || "Conteúdo em desenvolvimento..."}
                        </p>
                      </div>
                    )) : (
            ...
            <div className="p-6 pt-2 bg-black/20">
              ...
              <div className="mb-6 text-center px-4">
                <p className="text-sm text-gray-300 italic font-serif">
                  "Deus quis servir-se de Maria na Encarnação e quer servir-se dela na santificação das almas." 
                  <br /><span className="text-fiat-gold text-xs">(São Luís Maria de Montfort)</span>
                </p>
              </div>

              <div className="space-y-6">
                {marianDogmas.map(dogma => (
                  <div key={dogma.id} className="bg-white/5 p-4 rounded-xl border border-white/5 hover:border-fiat-gold/30 transition-colors">
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-10 h-10 bg-fiat-gold/10 rounded-full flex items-center justify-center border border-fiat-gold/20 shadow-[0_0_10px_rgba(212,175,55,0.15)]">
                        {getIcon(dogma.icon || 'Sparkles')}
                      </div>
